...
## 🚀 Key Features & Tools
- **`vscode_generate_mcp_config`**: Auto-compiles verified `.vscode/mcp.json` and `claude_desktop_config.json` files for zero-setup execution.
- **`vscode_audit_extension_manifest`**: Audits extension manifests for security vulnerabilities, activation triggers, and tool bindings.
- **`vscode_sync_workspace_settings`**: Synchronizes team settings, linters, and MCP permissions.
- **`vscode_inspect_live_gateways`**: Inspects real-time connection status across all 13 Cloudflare edge gateways directly from the IDE.
